...
do_get(Key, #{values := Values}) ->
    case maps:get(Key, Values, undefined) of
        undefined ->
            undefined;
        {Context, Value} ->
            {Context, mg_core_storage:binary_to_opaque(Value)}
    end.

-spec do_search(mg_core_storage:index_query(), state()) ->
...
-spec split_search_result(search_result(), mg_core_storage:index_limit()) ->
