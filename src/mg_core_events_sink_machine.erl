...
add_events(
    #{machine_id := EventSinkID} = Options,
    SourceNS,
    SourceMachineID,
    Events,
    ReqCtx,
...
-spec get_history(ns_options(), mg_core:id(), mg_core_events:history_range()) -> [event()].
get_history(Options, EventSinkID, HistoryRange) ->
    ...
    Batch = mg_core_dirange:fold(
        fun(EventID, Batch) ->
            Key = mg_core_events:add_machine_id(
                EventSinkID,
                mg_core_events:event_id_to_key(EventID)
            ),
...
-spec process_machine(Options, EventSinkID, Impact, PCtx, ReqCtx, Deadline, PackedState) ->
    Result
when
    Options :: ns_options(),
...
process_machine_(
    Options,
    EventSinkID,
    {call, {add_events, SourceNS, SourceMachineID, Events}},
    State
) ->
...
-spec store_sink_events(ns_options(), mg_core:id(), [event()]) -> ok.
...
-spec store_event(ns_options(), mg_core:id(), event()) -> ok.
store_event(Options, EventSinkID, SinkEvent) ->
...
-spec generate_sink_event_body(mg_core:ns(), mg_core:id(), mg_core_events:event()) -> event_body().
...
-spec sink_event_body_to_opaque(Vsn :: integer(), event_body()) -> mg_core_storage:opaque().
sink_event_body_to_opaque(_Vsn, #{
    source_ns := SourceNS,
    source_id := SourceMachineID,
    event := Event
}) ->
...
-spec opaque_to_sink_event_body(Vsn :: integer(), mg_core_storage:opaque()) -> event_body().
...
-spec sink_event_to_kv(mg_core:id(), event()) -> mg_core_storage:kv().
sink_event_to_kv(EventSinkID, Event) ->
    mg_core_events:add_machine_id(
        EventSinkID,
        mg_core_events:event_to_kv(Event, fun sink_event_body_to_opaque/2)
    ).

-spec kv_to_sink_event(mg_core:id(), mg_core_storage:kv()) -> event().
kv_to_sink_event(EventSinkID, Kvs) ->
    mg_core_events:kv_to_event(
        mg_core_events:remove_machine_id(EventSinkID, Kvs),
        fun opaque_to_sink_event_body/2
    ).
