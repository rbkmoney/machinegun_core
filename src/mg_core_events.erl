...
orient_range(R, forward) ->
    mg_core_dirange:align(R, {0, 1});
orient_range(R, backward) ->
    mg_core_dirange:align(R, {1, 0}).
...
update_events_range(undefined, NewLastEventID) ->
    mg_core_dirange:forward(get_next_event_id(undefined), NewLastEventID);
update_events_range(Range, NewLastEventID) ->
    mg_core_dirange:forward(mg_core_dirange:from(Range), NewLastEventID).
...
events_range_to_opaque(Range) ->
    {First, Last} = mg_core_dirange:bounds(Range),
    [First, Last].
...
opaque_to_events_range([First, Last]) ->
    mg_core_dirange:forward(First, Last).
...
-spec event_to_kv(event(T), fun((Vsn :: 1..2, T) -> mg_core_storage:opaque())) ->
    mg_core_storage:kv().
...
-spec kv_to_event(mg_core_storage:kv(), fun((Vsn :: 1..2, mg_core_storage:opaque()) -> T)) ->
    event(T).
...
events_to_kvs(Events, BodyToOpaque) ->
    [mg_core_events:event_to_kv(Event, BodyToOpaque) || Event <- Events].

-spec kvs_to_events([mg_core_storage:kv()], fun((Vsn :: 1..2, mg_core_storage:opaque()) -> T)) ->
    [event(T)].
...
-spec event_to_opaque(event(T), fun((Vsn :: 1..2, T) -> mg_core_storage:opaque())) ->
    mg_core_storage:opaque().
...
-spec add_machine_id
    ...
    (mg_core:id(), T) -> T when T :: mg_core_storage:key();
    (mg_core:id(), T) -> T when T :: [mg_core_storage:kv() | mg_core_storage:key()].
