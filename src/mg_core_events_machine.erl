...
-type event_sweeper() :: fun((mg_core_events:events_range()) -> [mg_core_events:event()]).
-type event_sources() :: [{mg_core_events:events_range(), event_sweeper()}, ...].
...
get_events(Sources, EventsRange, HRange) ->
    lists:flatten(sweep_events(Sources, mg_core_events:cull_range(EventsRange, HRange))).

-spec sweep_events(event_sources(), mg_core_events:events_range()) ->
    [mg_core_events:event() | [mg_core_events:event()]].
sweep_events([{SweepRange, Sweeper} | Sources], EvRange) when EvRange /= undefined ->
    case mg_core_events:intersect_range(EvRange, SweepRange) of
...
-spec event_list_sweeper([mg_core_events:event()]) ->
    event_sweeper().
...
-spec sweep_event_list(mg_core_events:events_range(), [mg_core_events:event()]) ->
    [mg_core_events:event()].
