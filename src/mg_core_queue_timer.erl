...
search_tasks(Options = #{timer_queue := TimerQueue}, Limit, State = #state{}) ->
    ...
    {Timers, Continuation} = mg_core_namespace:search(namespace_options_ref(Options), Query, Limit),
...
execute_task(Options, #{id := MachineID, target_time := Timestamp}) ->
    ...
    ok = mg_core_namespace:send_timeout(namespace_options_ref(Options), MachineID, Timestamp, Deadline).
