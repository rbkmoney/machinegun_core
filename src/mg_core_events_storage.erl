...
-type start_options() :: mg_core_events_machine:start_options().
-type call_options() :: mg_core_events_machine:call_options().

%%

-spec child_spec(start_options()) ->
    supervisor:child_spec().
child_spec(Options) ->
    mg_core_storage:child_spec(events_storage_options(Options), events_storage).

-spec store_event(start_options() | call_options(), mg_core:id(), mg_core_events:event()) ->
    ok.
store_event(Options, ID, Event) ->
    store_events(Options, ID, [Event]).

-spec store_events(start_options() | call_options(), mg_core:id(), [mg_core_events:event()]) ->
    ok.
...
-spec get_events(call_options(), mg_core:id(), mg_core_events:events_range()) ->
