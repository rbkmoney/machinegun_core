...
-type ref() :: {id, machinegun_core:id()} | {tag, mg_machine_tags:tag()}.
-type options() :: #{
...
-spec add_tag(options(), machinegun_core:id(), request_context(), deadline(), undefined | mg_machine_tags:tag()) ->
    ok.
...
-spec store_events(options(), machinegun_core:id(), request_context(), [mg_events:event()]) ->
    ok.
...
-spec push_events_to_event_sinks(options(), machinegun_core:id(), request_context(), deadline(), [mg_events:event()]) ->
    ok.
...
-spec machine(options(), machinegun_core:id(), state(), [mg_events:event()], mg_events:history_range()) ->
    machine().
...
-spec events_to_kvs(machinegun_core:id(), [mg_events:event()]) ->
    [mg_storage:kv()].
events_to_kvs(MachineID, Events) ->
    mg_events:add_machine_id(MachineID, mg_events:events_to_kvs(Events)).

-spec kv_to_event(machinegun_core:id(), mg_storage:kv()) ->
    mg_events:event().
