...
-spec check_chain(mg_core_machine:options(), id()) ->
    ok.
check_chain(Options, ID) ->
    _ = rand:seed(exsplus, {ID, ID, ID}),
    check_chain(Options, ID, 0, all_actions(), not_exists).

-spec check_chain(mg_core_machine:options(), id(), seq(), [action()], state()) ->
    ok.
% TODO убрать константы
...
-spec do_action(mg_core_machine:options(), id(), seq(), action()) ->
    result().
do_action(Options, ID, Seq, Action) ->
    try
        case Action of
            {start, ResultAction} ->
                mg_core_namespace:start(Options, id(ID), ResultAction, req_ctx(ID, Seq), mg_core_deadline:default());
            fail ->
                mg_core_namespace:fail(Options, id(ID), req_ctx(ID, Seq), mg_core_deadline:default());
            {repair, ResultAction} ->
                mg_core_namespace:repair(Options, id(ID), ResultAction, req_ctx(ID, Seq), mg_core_deadline:default());
            {call, ResultAction} ->
                mg_core_namespace:call(Options, id(ID), ResultAction, req_ctx(ID, Seq), mg_core_deadline:default())
        end
    catch
