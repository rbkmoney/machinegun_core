...
-spec direction_test(config()) ->
    _.
direction_test(_) ->
    ?assertEqual(+1, mg_core_dirange:direction({1, 10})),
    ?assertEqual(-1, mg_core_dirange:direction({10, 1})),
    ?assertEqual(-1, mg_core_dirange:direction(mg_core_dirange:backward(42, 42))),
    ?assertEqual(0, mg_core_dirange:direction(undefined)),
    ?assert(check_property(
        % Reversal changes direction
        ?FORALL(R, range(),
            equals(-mg_core_dirange:direction(R), mg_core_dirange:direction(mg_core_dirange:reverse(R)))
        )
    )).
...
-spec size_test(config()) ->
    _.
size_test(_) ->
    ?assertEqual(10, mg_core_dirange:size({1, 10})),
    ?assertEqual(10, mg_core_dirange:size({10, 1})),
    ?assertEqual(10, mg_core_dirange:size({-10, -1})),
    ?assert(check_property(
    ...
    ?assert(check_property(
        ...
        ?FORALL(R, range(), equals(mg_core_dirange:size(R), mg_core_dirange:size(mg_core_dirange:reverse(R))))
    )).
...
-spec dissect_test(config()) ->
    _.
dissect_test(_) ->
    ?assertEqual({undefined, undefined}, mg_core_dirange:dissect(undefined, 42)),
    ?assertEqual({{1, 10}, undefined}, mg_core_dirange:dissect({1, 10}, 42)),
    ?assertEqual({undefined, {10, 1}}, mg_core_dirange:dissect({10, 1}, 42)),
    ?assertEqual({{10, 1}, undefined}, mg_core_dirange:dissect({10, 1}, 1)),
    ?assertEqual({{10, 2}, {1, 1, -1}}, mg_core_dirange:dissect({10, 1}, 2)),
    ?assert(check_property(
...
-spec intersect_test(config()) ->
    _.
intersect_test(_) ->
    ?assertEqual({undefined, undefined, undefined}, mg_core_dirange:intersect(undefined, {1, 10})),
    ?assertEqual({{10, 7}, {6, 5}, {4, 1}}, mg_core_dirange:intersect({10, 1}, {5, 6})),
    ?assert(check_property(
        % Range intersects with itself with no left/right differences
        ?FORALL(R, nonempty_range(),
            equals({undefined, R, undefined}, mg_core_dirange:intersect(R, R))
        )
    )),
    ?assert(check_property(
        % Range intersects with reversal of itself with no left/right differences
        ?FORALL(R, nonempty_range(),
            equals({undefined, R, undefined}, mg_core_dirange:intersect(R, mg_core_dirange:reverse(R)))
        )
    )),
    ?assert(check_property(
        ...
        ?FORALL({R, With}, {range(), nonempty_range()}, begin
            ...
            conjunction([
                {strictly_left_diff,
                    equals({LD, undefined, undefined}, mg_core_dirange:intersect(LD, With))
                },
                {strictly_right_diff,
                    equals({undefined, undefined, RD}, mg_core_dirange:intersect(RD, With))
                }
            ])
        end)
    )),
    ?assert(check_property(
        ...
        ?FORALL({R0, RWith}, {range(), nonempty_range()}, begin
            ...
            equals(mg_core_dirange:size(R0), lists:sum([mg_core_dirange:size(R) || R <- [RL, RI, RR]]))
        end)
    )),
    ?assert(check_property(
...
-spec enumerate_test(config()) ->
    _.
enumerate_test(_) ->
    ?assertEqual([1, 2, 3, 4, 5], mg_core_dirange:enumerate({1, 5})),
    ?assertEqual([5, 4, 3, 2, 1], mg_core_dirange:enumerate({5, 1})),
    ?assert(check_property(
    ...
    ?assert(check_property(
        ...
        ?FORALL(R, range(),
            equals(lists:reverse(mg_core_dirange:enumerate(R)), mg_core_dirange:enumerate(mg_core_dirange:reverse(R)))
        )
    )),
    ?assert(check_property(
...
fold_test(_) ->
    ?assert(check_property(
        ...
        ?FORALL(R, range(),
            equals(mg_core_dirange:enumerate(R), mg_core_dirange:fold(fun (E, L) -> L ++ [E] end, [], R))
        )
    )).
