...
base_test(C) ->
    ...
    hello   = mg_core_workers_manager:call(Options, 42, hello, ?req_ctx, mg_core_deadline:default()),
...
load_fail_test(C) ->
    ...
    {error, {unexpected_exit, _}} =
        mg_core_workers_manager:call(Options, 42, hello, ?req_ctx, mg_core_deadline:default()),
    ok      = wait_machines_unload(?unload_timeout),
    ok      = stop_workers(Pid).
...
load_error_test(C) ->
    ...
    Pid     = start_workers(Options),
    {error, test_error} = mg_core_workers_manager:call(Options, 42, hello, ?req_ctx, mg_core_deadline:default()),
    ok      = wait_machines_unload(?unload_timeout),
    ok      = stop_workers(Pid).
...
call_fail_test(C) ->
    ...
    Pid     = start_workers(Options),
    {error, {unexpected_exit, _}} =
        mg_core_workers_manager:call(Options, 43, hello, ?req_ctx, mg_core_deadline:default()),
    ok      = wait_machines_unload(?unload_timeout),
    ok      = stop_workers(Pid).
...
unload_loading_test(C) ->
    ...
    Pid     = start_workers(Options),
    {error, {timeout, _}} = mg_core_workers_manager:call(Options, 42, hello, ?req_ctx, mg_core_deadline:from_timeout(LoadLag div 2)),
    WorkerPid = wait_worker_pid(42),
...
-spec stress_test_do_test_call(mg_core_workers_manager:options(), pos_integer(), retry_strategy()) ->
    ok.
...
-spec manager_contention_test_call(mg_core_workers_manager:options(), pos_integer(), retry_strategy()) ->
    ok.
...
-spec stress_test_start_process(mg_core_workers_manager:options(), load_job_fun(), _N :: pos_integer(), _State) ->
    pid().
